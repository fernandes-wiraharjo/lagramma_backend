...
class PaymentController extends Controller
{
    ...
    public function approve(Request $request, $invoiceNo)
    {
        ...
        try {
            ...
            ->update([
                'status' => 'APPROVED',
                'payment_method' => 'Bank Transfer (Manual Confirmation)',
                'updated_by' => auth()->id()
            ]);
            ...
            DB::rollBack();
            Log::error('Error payment approval on invoice no: ' . $invoiceNo . ', ' . $e->getMessage());
            return response()->json(['status' => 'error', 'message' => 'Failed to approve payment.'], 500);
        }
    ...
    public function reject(Request $request, $invoiceNo)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('Error payment rejection on invoice no: ' . $invoiceNo . ', ' . $e->getMessage());
            return response()->json(['status' => 'error', 'message' => 'Failed to reject payment.'], 500);
        }

        return response()->json(['status' => 'success', 'message' => 'Payment rejected successfully.']);
    }
}
